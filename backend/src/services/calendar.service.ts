...
export class CalendarService {
  ...
  static async generateIcsFeedByToken(token: string): Promise<string | null> {
    ...
    const lines: string[] = [
      "BEGIN:VCALENDAR",
      "VERSION:2.0",
      "PRODID:-//Todolist-App//Calendar Feed//ID",
      "CALSCALE:GREGORIAN",
      "METHOD:PUBLISH",
      `X-WR-CALNAME:${this.escapeIcsText(`Todolist-App Tasks - ${user.name}`)}`,
      "X-WR-CALDESC:Jadwal tugas aktif dan tenggat waktu dari Todolist-App",
      "X-WR-TIMEZONE:UTC",
