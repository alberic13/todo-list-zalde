...
import { RagLocalFallbackService } from "./ragLocalFallback.service";
import type { ChatWithRagResult, ScoredTaskItem } from "./rag.types";

export type { ChatWithRagResult, ScoredTaskItem } from "./rag.types";

// ponytail: RAG orchestrator for postgres vector similarity and gemini context synthesis.
export class RagService {
  /**
   * Fast vector semantic search directly in PostgreSQL (pgvector HNSW)
   */
  static async semanticSearch(userId: string, query: string, topK = 10): Promise<ScoredTaskItem[]> {
...
Jawab dalam Bahasa Indonesia yang singkat, ramah, padat, dan langsung actionable, kamu boleh bantu untuk kirim ringkasan jadwal hari ini ke whatsapp, bantu rencanakan jadwal tugas minggu ini jangan mau menjawab ketika ada pertanyaan di luar konteks to do list ini.`;
    ...
    if (!GeminiClient.getApiKey() || aiResponse.startsWith("⚠️") || aiResponse.startsWith("Maaf,")) {
      aiResponse = RagLocalFallbackService.generateSmartLocalResponse(
        activeTasks,
        message,
        userName,
        aiResponse.startsWith("⚠️") ? aiResponse : undefined
      );
    }
  ...
  static async breakdownTask(title: string, description?: string): Promise<string[]> {
    if (!GeminiClient.getApiKey()) {
      return RagLocalFallbackService.generateSmartBreakdown(title, description);
    }
