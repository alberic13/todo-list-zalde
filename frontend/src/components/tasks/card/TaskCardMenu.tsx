...
export const TaskCardMenu: React.FC<TaskCardMenuProps> = ({
  ...
  return (
    <div className="relative" ref={triggerRef}>
      ...
      {showMenu && (
        <>
          <div className="fixed inset-0 z-30" onClick={(e) => { e.stopPropagation(); updateMenuState(false); }} />
          <div
            ref={menuRef}
            onClick={(e) => e.stopPropagation()}
            className={`absolute right-0 ${openUpwards ? "bottom-full mb-1" : "top-full mt-1"} w-44 rounded-2xl bg-white/95 backdrop-blur-xl shadow-2xl border border-slate-200/90 py-1.5 z-40 animate-in fade-in zoom-in-95 duration-150`}
          >
            <button
              type="button"
              onClick={() => { updateMenuState(false); onOpenChat?.(task); }}
            ...
            <button
              type="button"
              onClick={() => { updateMenuState(false); onEdit(task); }}
            ...
            {availableMoveOptions.map((opt) => (
              <button
                key={opt.status}
                type="button"
                onClick={() => { updateMenuState(false); onStatusChange?.(task.id, opt.status); }}
